...
public class Crush extends Configured implements Tool {
    ...
    Options buildOptions() {
        ...
        option = OptionBuilder
                .withDescription("Debug logging to console.")
                .withLongOpt("debug")
                .create();
        options.addOption(option);
...
LOG.info("MAX TASKS V1 " + job.get("mapreduce.job.reduces") + " " + job.get("mapred.reduce.tasks"));
...
LOG.info("MAX TASKS V2 " + job.get("mapreduce.job.reduces") + " " + job.get("mapred.reduce.tasks"));
                job.set("crush.args.style", "v2");
                /*
                 * V2 style arguments.
                 */
        ...
        if (codec == null)
            codec = "deflate";
        LOG.debug(format("Codec option: %s", codec));
        ...
        LOG.debug(format("Codec class name: %s", codecClassName));
    ...
    public int run(String[] args) throws Exception {
        ...
        if (Mode.STAND_ALONE == mode) {
            job.set("crush.job.mode", "standalone");
            standAlone();
        } else {
            job.set("crush.job.mode", "normal");
            writeDirs();
    ...
    private void standAlone() throws IOException {
         ...
         */
        job.set("mapreduce.task.id",  "task_000000000000_00000_r_000000");
        job.set("mapreduce.task.attempt.id", "attempt_000000000000_0000_r_000000_0");

        job.set("mapreduce.output.fileoutputformat.outputdir", absOutDir);
...
LOG.info("EMPTY " + filePath + "(" + fileLength + ")");
                                        removableFiles.add(filePath);
                                    } else {
LOG.info("CRUSHABLE " + filePath + "(" + fileLength + ")");
                                        crushables.add(content);
                                        crushableBytes += fileLength;
                                    }
                                } else {
LOG.info("REMOVABLE " + filePath + "(" + fileLength + ")");
                                }
                            }
                        }
                        ...
                        if (0 == crushableBytes) {
                             ...
                             */
LOG.info("CRUSHABLE BYTES: " + crushableBytes);
