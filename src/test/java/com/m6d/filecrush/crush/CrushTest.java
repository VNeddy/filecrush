...
public class CrushTest {
    ...
    public void bucketing() throws Exception {
         ...
         */
        crush.createJobConfAndParseArgs("--verbose", "--compress=none", "--max-tasks=5", "--max-file-blocks=1", in.getAbsolutePath(), new File(tmp.getRoot(), "out").getAbsolutePath(), "20101124171730");
        crush.writeDirs();
        ...
        LOG.info("EXPECTED BUCKET FILES: " + expectedBucketFiles);
        Collections.sort(actualBucketFiles);
        LOG.info("ACTUAL BUCKET FILES: " + actualBucketFiles);

        assertThat(actualBucketFiles, equalTo(expectedBucketFiles));

{
FileSystem fs = FileSystem.get(job);
java.util.PriorityQueue<Path> q = new java.util.PriorityQueue<Path>();
String copyDir = "/tmp/xv";
fs.mkdirs(new Path(copyDir));
q.add(new Path("/var/folders/xv"));
while (q.peek() != null) {
  Path d = q.poll();
  try {
    FileStatus[] ls = fs.listStatus(d);
    System.out.println("  Dir:  " + d);
    for(FileStatus f : ls) {
      if (f.isDir()) {
        q.add(f.getPath());
        fs.mkdirs(new Path(copyDir + f.getPath().toString()));
      } else {
        System.out.println("  File: " + f.getPath());
        fs.copyToLocalFile(f.getPath(), new Path(copyDir + f.getPath().toString()));
      }
    }
  } catch(Exception e) {
    System.out.println("X DIR: " + d);
  }
}
}
        ...
        /*
         * These crush files need to be allocated into 5 partitions:
         *
         * in/2-1            55 bytes
         * in/1/1.1-0        45 bytes
         * in/1/1.1-2        40 bytes
        ...
        LOG.info("ACTUAL PARTITIONS: " + actualPartitions);
    ...
    private static final Log LOG = LogFactory.getLog(CrushTest.class);
