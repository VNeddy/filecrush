...
public class CrushMapReduceTest {
    private static final Log LOG = LogFactory.getLog(CrushMapReduceTest.class); 
    ...
    private static MiniMRClientCluster yarn;

    private static int lastAttemptedPort = 30000;

    final private static int numDataNodes = 1;
    final private static int numNodeManagers = 1;
    final private static int nnPort = 0;
    final private static StartupOption dfsOpts = StartupOption.FORMAT;
    private static int rmPort;
    private static int rmResourceTrackerPort;
    private static int rmSchedulerPort;
    private static int rmAdminPort;
    private static int rmWebappPort;
    ...
    public static void setUpClass() throws IOException, URISyntaxException {
        ...
        rmPort = getNextAvailablePort();
        rmResourceTrackerPort = getNextAvailablePort();
        rmSchedulerPort = getNextAvailablePort();
        rmAdminPort = getNextAvailablePort();
        rmWebappPort = getNextAvailablePort();

        dfs = new MiniDFSCluster.Builder(jobConf)
            .nameNodePort(nnPort)
        ...
        String fs = dfs.getFileSystem().getUri().toString();
        LOG.info("MiniDFSCluster default URI: " + fs);

        FileSystem.setDefaultUri(jobConf, new URI(fs));
        jobConf.setBoolean(YarnConfiguration.YARN_MINICLUSTER_FIXED_PORTS, true);
        jobConf.setBoolean(JHAdminConfig.MR_HISTORY_MINICLUSTER_FIXED_PORTS, false);
        jobConf.set(YarnConfiguration.RM_ADDRESS, MiniYARNCluster.getHostname()
            + ":" + rmPort);
        jobConf.set(YarnConfiguration.RM_RESOURCE_TRACKER_ADDRESS, MiniYARNCluster.getHostname()
            + ":" + rmResourceTrackerPort);
        jobConf.set(YarnConfiguration.RM_SCHEDULER_ADDRESS, MiniYARNCluster.getHostname()
            + ":" + rmSchedulerPort);
        jobConf.set(YarnConfiguration.RM_ADMIN_ADDRESS, MiniYARNCluster.getHostname()
            + ":" + rmAdminPort);
        jobConf.set(YarnConfiguration.RM_WEBAPP_ADDRESS, MiniYARNCluster.getHostname()
            + ":" + rmWebappPort);
        yarn = MiniMRClientClusterFactory.create(CrushMapReduceTest.class, numNodeManagers,
            jobConf);
        LOG.info("Started MiniMRCluster");
  
        LOG.info(String.format("Cluster information:\n\n\nNN port: %d\nRM port: %d\nRM Admin port: %d\nRM Webapp port: %d\nRM Scheduler port: %d\nRM Resource Tracker port: %d\n\n\n",
           dfs.getNameNodePort(), rmPort, rmAdminPort, rmWebappPort, rmSchedulerPort, rmResourceTrackerPort));

        try {
            TMP.before();
            LOG.info("Temporary Temp location = " + TMP.getRoot().getAbsolutePath());
        } catch (Throwable e) {
    ...
    public static void tearDownClass() {
        ...
        if (yarn != null) {
            try {
                LOG.info("Shutting down YARN");
                yarn.stop();
                LOG.info("YARN has been shut down");
            } catch (IOException e) {
                throw new RuntimeException("Could not shutdown the YARN cluster", e);
            }
        }
        if (dfs != null) {
            LOG.info("Shutting down HDFS");
            dfs.shutdown();
            LOG.info("HDFS has been shut down");
        }
    }
    ...
    public void setUp() throws Exception {
        ...
        jobConf.set("mapreduce.output.fileoutputformat.compress.type", CompressionType.BLOCK.name());
        jobConf.set("mapreduce.output.fileoutputformat.compress.codec", CustomCompressionCodec.class.getName());

        /*
         * Ensure we start with a clean slate
         */
        LOG.info("Preparing homedir " + homeDir);
        FileSystem fs = dfs.getFileSystem();
        Path homeDirPath = fs.makeQualified(new Path("."));
        homeDir = homeDirPath.toUri().getPath();
        fs.delete(homeDirPath, true);
        fs.mkdirs(homeDirPath);
    ...
    public void tearDown() throws Exception {
         ...
         */
        FileStatus[] tmpFiles = dfs.getFileSystem().listStatus(new Path("tmp"));
...
//
    ...
    private void writeFile(String fileName, Format format) throws IOException {
        ...
        if (Format.TEXT == format) {
            PrintWriter writer = new PrintWriter(new BufferedWriter(new PrintWriter(dfs.getFileSystem().create(path, false))));
            ...
            Writer writer = SequenceFile.createWriter(dfs.getFileSystem(), dfs.getFileSystem().getConf(), path, CustomWritable.class,
    ...
    private void writeHugeFile(String fileName, long length) throws IOException {
        Path path = new Path(fileName);
        OutputStream writer = dfs.getFileSystem().create(path, false);
...
//    }
    ...
    private static int getNextAvailablePort() {
        while (++lastAttemptedPort < 65535) {
            try {
                ServerSocket s = new ServerSocket(lastAttemptedPort);
                s.close();
                return lastAttemptedPort;
